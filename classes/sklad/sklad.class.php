...
if (($params['db']!=null) && (is_object($params['db'])))
	...
	$this->components['options'] = $this->components['factory']->createInstance("WiseMonkey", $params); // Подключаем шаблонизатор
	$this->components['paginator'] = $this->components['factory']->createInstance("Paginator", $params); // Подключаем шаблонизатор	
	$this->components['formitems'] = $this->components['factory']->createInstance("FormItems", $params); // Подключаем шаблонизатор	
        $this->components['usr'] = $this->components['factory']->createInstance("Meerkat", $params); // Подключаем шаблонизатор	
}
  /** 
  * Деструктор
  */
 ...
 * Заполняет шаблоны /classes/hamster/urlitem.tpl
 * и /classes/hamster/sitemap.tpl
 ...
 */ 
  ...
  function CreateSitemap() // Создает карту интернет-магазина
 {
	 $sitemap = "";
     $urls = "";
     $this->components['view']->UseTpl($_SERVER['DOCUMENT_ROOT'].'/classes/hamster/urlitem.tpl');
	 // Для групп товаров
	 $this->components['db']->setTable('trade_structure');
	 $this->components['db']->Select('*', '1=1');
	 while ($data = @$this->components['db']->Read())
     ...
     // Пишем карту сайта
    $this->components['view']->UseTpl($_SERVER['DOCUMENT_ROOT'].'/classes/hamster/sitemap.tpl');
