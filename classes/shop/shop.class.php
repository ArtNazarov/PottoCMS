...
if (($params['db']!=null) && (is_object($params['db'])))
	...
	$this->components['search'] = $this->components['factory']->createInstance("Scorpio", $params); // Подключаем шаблонизатор	
	$this->components['cache'] = $this->components['factory']->createInstance("Forest", $params); // Кэш
        $this->components['usr'] = $this->components['factory']->createInstance("Meerkat", $params); // Кэш
        $this->components['gb'] = $this->components['factory']->createInstance("Entropia", $params); // Глобальные блоки
