...
while ($data = $this->components['db']->Read())
    ...
    $this->components['log']->WriteLog('jerboa', 'plugstyles done');
   }
...
{return $_GET['id']; };
 ...
 $this->user_area(); 
    ...
    $page['TRANSLATIONS'] = $this->GetTranslationsLinks($d['id']);    
	 ...
	 $this->components['view']->Publish();
         $this->components['log']->MyLog('jerboa', 'viewpage end');
	}
 ...
 {		$this->site_closed_msg(); }
	...
	function Run()
	{
	  $this->doAction($this->getTask());
          $this->components['log']->WriteLog('jerboa', 'run - OK');
	}
